 ...
 * @see     ClientSSH
 */
public class ServerSSH extends SSH implements Runnable {
    ...
    public ServerSSH(Socket sshSocket,
    ...
    public void run() {
        try {
            ...
            authenticateUser();
            controller = new ChannelController(this, sshIn, sshOut,
                                                  sndCipher, 
                                                  rcvCipher, 
                                                  null, true);
            receiveOptions();
            ...
            // !!!
            log("error in IESTTunnel: " + e);
        }
    }

    static RSAPrivateCrtKey getPrivate(RSAKeyFileSSH keyFile) {
    ...
    public static void sshd(int port) throws IOException {
        ...
        keyFile    = new RSAKeyFileSSH(hostKeyFile);
        ...
        try {
            ServerSSH dummy = new ServerSSH(null, PROTOFLAG_HOST_IN_FWD_OPEN, 0, 0, null, null);
        ...
        System.out.println("starting new IESTTunnel on port " + port + "...");

        while(keepRunning) {
            Socket sshSocket = listenSock.accept();
            ServerSSH srv = new ServerSSH(sshSocket, PROTOFLAG_HOST_IN_FWD_OPEN, 0xff, 0x0f,
                                          serverKey, hostKey);
    ...
    void negotiateVersion() throws IOException {
        ...
        if(cliVersionMajor > 1) {
            throw new IOException("IESTTunnel do not support SSHv2 yet, can only serve SSHv1 client");
    ...
    void sendServerData() throws IOException {
        PduOutputStreamSSH pdu =
            new PduOutputStreamSSH(SMSG_PUBLIC_KEY, null, rand);
    ...
    void receiveSessionKey() throws IOException {
        PduInputStreamSSH inpdu =
            new PduInputStreamSSH(CMSG_SESSION_KEY, null);
    ...
    void authenticateUser() throws IOException {
        ...
        PduInputStreamSSH inpdu = new PduInputStreamSSH(CMSG_USER,
                                  rcvCipher);
        String            user;
    ...
    boolean doRSAAuth(String userName, BigInteger pubKeyN) throws IOException {
        RSAPublicKeyFileSSH keyFile;
        PduOutputStreamSSH  outpdu;
        PduInputStreamSSH   inpdu;
        boolean             authenticated = false;

        keyFile = RSAPublicKeyFileSSH.loadFromFile(authKeysDir + userName, false);
        ...
        outpdu = new PduOutputStreamSSH(SMSG_AUTH_RSA_CHALLENGE,
                                        sndCipher, rand);
        outpdu.writeBigInteger(enc);
        outpdu.writeTo(sshOut);

        inpdu = new PduInputStreamSSH(CMSG_AUTH_RSA_RESPONSE, rcvCipher);
        inpdu.readFrom(sshIn);
        tmp = new byte[16];
        inpdu.read(tmp, 0, 16);
    ...
    void receiveOptions() throws IOException {
        PduInputStreamSSH pdu;
        boolean           finished = false;

        while(!finished) {
            pdu = new PduInputStreamSSH(MSG_ANY, rcvCipher);
            pdu.readFrom(sshIn);
            switch(pdu.type) {
    ...
    void sendResult(int type) throws IOException {
        PduOutputStreamSSH pdu;
        pdu = new PduOutputStreamSSH(type, sndCipher, rand);
        pdu.writeTo(sshOut);
    }
