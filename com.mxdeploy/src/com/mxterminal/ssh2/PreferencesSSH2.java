...
public class PreferencesSSH2 {
    ...
    public void readFrom(TransportPDUSSH2 pdu) {
        for(int i = 0; i < 10; i++) {
            setPreference(kexFields[i], pdu.readJavaString());
        }
    }

    public void writeTo(TransportPDUSSH2 pdu) {
        for(int i = 0; i < 10; i++) {
    ...
    public boolean isSupported(String type, String item) {
        ...
        return ListUtilSSH2.isInList(list, item);
    }

    public KeyExchanger
    selectKEXAlgorithm(PreferencesSSH2 peerPrefs, boolean weAreAServer)
    throws KEXFailedExceptionSSH2 {
        KeyExchanger kexImpl = null;
        ...
        sameKEXGuess = ListUtilSSH2.getFirstInList(cliKEXList).equals
            (ListUtilSSH2.getFirstInList(srvKEXList));

        String hostKeyAlgorithm = null;
        String kexAlgorithm = ListUtilSSH2.getFirstInList(cliKEXList);
        while (kexAlgorithm != null) {
            if (srvKEXList.indexOf(kexAlgorithm) != -1) {
                kexImpl          = KeyExchanger.getInstance(kexAlgorithm);
                hostKeyAlgorithm = chooseHostKeyAlgorithm
                    (cliHKAList, srvHKAList, kexImpl.getHostKeyAlgorithms());
                if (hostKeyAlgorithm != null)
                    break;
            }
            cliKEXList = ListUtilSSH2.removeFirstFromList(cliKEXList, kexAlgorithm);
            kexAlgorithm = ListUtilSSH2.getFirstInList(cliKEXList);
        }
        ...
        if(kexAlgorithm == null) {
            ...
            throw new KEXFailedExceptionSSH2(msg);
        }
    ...
    public boolean canAgree(PreferencesSSH2 peerPrefs, boolean weAreAServer) {
        haveAgreed = true;
        /*
    ...
    private String chooseHostKeyAlgorithm(String cliHKAList, String srvHKAList,
                                          String kexHKAList) {
        String alg = ListUtilSSH2.chooseFromList(cliHKAList, kexHKAList);
        while(alg != null && !ListUtilSSH2.isInList(srvHKAList, alg)) {
            cliHKAList = ListUtilSSH2.removeFirstFromList(cliHKAList, alg);
            alg        = ListUtilSSH2.chooseFromList(cliHKAList, kexHKAList);
        }
        return alg;
    }

    private String choosePref(String type, PreferencesSSH2 peerPrefs,
                              boolean weAreAServer) {
        ...
        return ListUtilSSH2.chooseFromList(clientList, serverList);
    }
