...
public class ConsoleScreen {
    ...
    protected ComponentView display = null;
    
    private int colsMax = 0;
    private int oldColsMax = 0;
    ...
    public ConsoleScreen(int rows, int cols, char defultChar, int defaultAttrib) {
        curAttr       = ComponentModel.ATTR_CHARDRAWN;
        curRow        = 0;
        curCol        = 0;
        visTop        = 0;
    ...
    public void writeLineDrawChar(char c) {
        writeChar(c, curAttr | ComponentModel.ATTR_LINEDRAW, false);
    }
    ...
    public synchronized void writeChar(char c, int attr, boolean insert) {
        ...
        if (doublewidth) {
            attributes[idxRow][curCol-1] |= ComponentModel.ATTR_DWIDTH_L;
            if (attributes[idxRow].length > curCol)
                attributes[idxRow][curCol++] = ComponentModel.ATTR_DWIDTH_R;
        }
        setCursorPosition(curRow, curCol);
    ...
    private static int ATTR_MASK = ComponentModel.MASK_BGCOL |
                                   ComponentModel.MASK_FGCOL |
                                   ComponentModel.ATTR_BGCOLOR |
                                   ComponentModel.ATTR_FGCOLOR;
    ...
    public void setForegroundColor(int c) {
        ...
        if(c >= 0 && c < 8) {
            if((curAttr & ComponentModel.ATTR_BOLD) != 0)
                c += 8;
            curAttr &= ~(ComponentModel.ATTR_FGCOLOR | ComponentModel.MASK_FGCOL);
            curAttr |= (ComponentModel.ATTR_FGCOLOR | (c << ComponentModel.SHIFT_FGCOL));
        } else {
            curAttr &= ~ComponentModel.ATTR_FGCOLOR;
        }
    }
    ...
    public void setBackgroundColor(int c) {
        ...
        if(c >= 0 && c < 8) {
            curAttr &= ~(ComponentModel.ATTR_BGCOLOR | ComponentModel.MASK_BGCOL);
            curAttr |= (ComponentModel.ATTR_BGCOLOR | (c << ComponentModel.SHIFT_BGCOL));
        } else {
            curAttr &= ~ComponentModel.ATTR_BGCOLOR;
        }
    }
    ...
    public synchronized void resize(int newRows, int newCols) {
    	...
    	//if( (newCols >= colsMax )||(newRows != rows) ) {
            ...
            for(int i = 0; i < copyRows; i++) {
                System.arraycopy(oldScreen[i+startAt], 0, screen[i], 0, oldColsMax);
                System.arraycopy(oldAttributes[i+startAt], 0, attributes[i], 0, oldColsMax);                    
    ...
    public void setSelection(int anchorRow, int anchorCol,
        ...
        if ((attributes[selectTopRow][selectTopCol]
             & ComponentModel.ATTR_DWIDTH_R) != 0) {
            selectTopCol--;
        }
        if ((attributes[selectBottomRow][selectBottomCol]
             & ComponentModel.ATTR_DWIDTH_L) != 0) {
            selectBottomCol++;
        }
        hasSelection = true;
        if (display != null) {
    ...
    protected synchronized String getContents(
        ...
        if (startRow != endRow) {
            for(i = startCol; i < cols; i++) {
                if ((attributes[startRow][i]&ComponentModel.ATTR_DWIDTH_R) != 0){
                    continue;
            ...
            for(i = startRow + 1; i < endRow; i++) {
                for(j = 0; j < cols; j++) {
                    if ((attributes[i][j] & ComponentModel.ATTR_DWIDTH_R) != 0) {
                        continue;
            ...
            for(i = 0; i <= endCol; i++) {
                if ((attributes[endRow][i]&ComponentModel.ATTR_DWIDTH_R) != 0) {
                    continue;
    ...
    public synchronized  ConsoleSearchContext search(
        ConsoleSearchContext lastContext, String key,
...
foundIt:
        ...
        if(found) {
            return new ConsoleSearchContext(i, j, i, j + len - 1);
        }
        return null;
    }
